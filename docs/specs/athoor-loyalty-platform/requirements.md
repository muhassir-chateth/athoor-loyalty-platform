...
- **A16 — Referral attribution is claimed, not inferred:** Shopify's `customers/create` payload carries no referral field, so attribution cannot come from the webhook. The friend submits the referral code themselves over the **signature-verified App Proxy** surface (`POST /v1/referral`), and the customer identity is taken from the verified `logged_in_customer_id` — never from the request body. Because a Shopify account must exist before the friend can be authenticated, the +150 signup reward is credited **when the code is claimed** rather than at the instant the account is created; eligibility is bounded by the no-prior-paid-purchase rule so attribution cannot be applied retrospectively.
...
- **A17 — Daily encrypted logical backups instead of point-in-time recovery:** The zero-cost database tier provides **no automated backups, no point-in-time recovery and no write-ahead-log retention**, so the original Requirement 13.6 was unmet on every clause. The delivered standard is instead a **daily logical dump, encrypted to a public key whose private half never enters CI or the repository**, retained off-site for at least 7 days. This yields an **RPO of approximately 24 hours and a manual RTO** — a mistake cannot be undone as of the moment before it, only as of the previous dump. The deviation is deliberate and **machine-checked**: the stricter PITR standard remains encoded and is asserted to fail, so it cannot be quietly forgotten. Backups contain customer PII (`customers.email`), which is why encryption is a requirement rather than a precaution. A total database loss without a usable dump leaves **balances approximately reconstructable** from Shopify orders and the per-change-refreshed `loyalty.*` metafields via the existing M0/M1 tooling, but **ledger history, audit trail, manual adjustments, referral attribution, FIFO lot structure and spend-to-code linkage are permanently lost**. Adoption triggers for real PITR are recorded in the backup runbook (criterion 13.6b).

- **A18 — Hardcoded rule constants are the MVP source of truth:** Requirement 21's configuration criteria are **structural** — the `markets` / `earning_rule_sets` / `reward_rule_sets` tables and `DbMarketConfigProvider` exist, are seeded, are tested, and are the forward path for a second Market — but the engine reads the hardcoded GBP constants at MVP. Wiring the provider into the earning and redemption paths was evaluated and declined (task 32): it would touch roughly eleven call sites including the redemption engine, require a cache with a refresh policy, and introduce a new failure mode in which a malformed configuration row aborts an earning or redemption transaction — all to make four numbers editable without a deploy, for a single GBP market with no second market planned. The cost of the deviation is that a threshold, multiplier or reward change requires a deploy. Its hazard — two representations of the same rules with only one obeyed — is **machine-checked**: the configured rows are compared against the constants and the result is published on `/health` (criterion 21.6a), so a hand-edited or stale rule-set row is observable instead of quietly misleading whoever reads it.**

- **A19 — The `app` Channel is unreachable until a native app exists:** `channel: "app"` requires a Customer_Account_API bearer token, and no token verifier is wired: `UnconfiguredTokenVerifier` refuses every token, so **every production request is attributed to `web`**. This is deliberate — Requirement 19 is explicitly future-scoped for delivered app features, no native app exists, and criterion 11.5 forbids building custom authentication, so refusing a token is more honest than inventing a check for it. Wiring a real verifier was evaluated and declined (task 42): it needs the shop's Customer_Account_API issuer configuration **and an authenticated storefront customer session to validate against** — the same access that blocks tasks 27 and 43 — and it would grant nothing, because no reward or Benefit is configured app-exclusive today. The consequence is that the app-exclusive gate (criterion 19.4, Property 15) is exercised by tests only and has never run in production. Its hazard — an app-exclusive item being grantable to **nobody**, silently, after a configuration edit — is **machine-checked** and published on `/health` (criterion 19.7a).**

- **A20 — The recorded booking request gains an operator lifecycle, and `declined` is an addition to it:** Requirement 18 criterion 5 required only that a consultation booking request be **recorded**; it said nothing about that request ever being read or advanced, which is why a recorded request sat operationally invisible — a member could submit one and no staff surface existed on which to see it, answer it, or close it. The delivered lifecycle (criteria 5a–5d) is `requested | confirmed | fulfilled | declined | cancelled`, with `fulfilled`, `declined` and `cancelled` terminal, transitions idempotent and mutually exclusive under concurrency, and every real status change writing an immutable audit record. `declined` is an **addition beyond the status set described in `design.md`**, which lists four statuses and includes `confirmed` but not `declined`: without it an operator closing a request that will not be honoured must either leave it indefinitely `requested` or misrepresent it as `fulfilled`, so the designed status set had no honest terminal state for a refusal. The audit trail's permitted operation types are correspondingly extended to include `benefit_request` under Requirement 10 criterion 9, whose generic wording ("the operation type") is unchanged. This addition **enables no seeded perk**: A13 still holds, all six seeded Benefits remain `active = false`, and enabling one is a business decision.**
...
9a. THE Loyalty_Service SHALL capture referral attribution from the friend, over the signature-verified App Proxy surface, as a submitted referral code resolved to the referrer; and SHALL NOT infer attribution from customer tags, notes, or any unverified request field. *(A16)*
9b. IF a referral code is submitted by a customer who already has a prior paid purchase, THEN THE Loyalty_Service SHALL reject the submission, create no referral record and no earning, and leave all balances unchanged.
9c. WHEN a customer is enrolled, THE Loyalty_Service SHALL assign that customer a unique referral code, and SHALL preserve any code already assigned so a replayed enrolment issues no new code.
...
6. THE Loyalty_Service SHALL take an automated backup of the PostgreSQL database at least once every 24 hours, SHALL encrypt every backup artifact at rest, and SHALL retain each artifact for at least 7 days. *(A17 — amended from the original PITR requirement; the zero-cost database tier provides no automated backups, no point-in-time recovery and no write-ahead-log retention, so the delivered standard is a daily encrypted logical dump with an RPO of approximately 24 hours and a manual RTO)*
6a. WHERE an automated backup completes successfully, THE Loyalty_Service SHALL record the completion durably and SHALL report the age of the most recent successful backup on its health endpoint, so that a backup mechanism which has stopped running is externally detectable rather than silent.
6b. WHILE the platform remains on a database tier without point-in-time recovery, THE Loyalty_Service SHALL treat criterion 6 as the operative standard and SHALL keep the point-in-time-recovery standard machine-checkable so the deviation cannot be silently forgotten; and WHEN any of the adoption triggers recorded in the backup runbook is met — a 24-hour loss of Ledger activity becoming non-reconstructable by hand, redemption volume making spend-to-code linkage non-reconstructable by hand, or the service moving off free hosting for any other reason — THE Loyalty_Service SHALL enable point-in-time recovery with write-ahead-log retention of at least 7 days and SHALL be verified against that stricter standard thereafter.
...
5a. WHERE a booking request has been recorded, THE Loyalty_Service SHALL make that request available to an Admin_User for review and SHALL support advancing it through a defined lifecycle whose statuses are `requested`, `confirmed`, `fulfilled`, `declined`, and `cancelled`, of which `fulfilled`, `declined`, and `cancelled` are terminal. *(A20)*
5b. IF an Admin_User attempts a transition out of a terminal status, THEN THE Loyalty_Service SHALL refuse the transition, perform no state change, and return a response reporting the booking request's current status and the requested status.
5c. WHEN a transition is requested into the status a booking request already holds, THE Loyalty_Service SHALL treat the transition as a no-op, perform no state change, and create no audit-trail record.
5d. WHEN a booking request status change is applied, THE Loyalty_Service SHALL create an immutable audit-trail record of that change consistent with Requirement 10 criterion 9.
...
7a. **(Added, task 42 — A19.)** WHILE no Customer_Account_API token verifier is configured, the `app` Channel is unreachable and every request is attributed to `web`. THE Loyalty_Service SHALL publish, read-only, which Channels a request can be attributed to and any configured reward or Benefit that requires an unreachable Channel and is therefore grantable to no customer. Such a finding SHALL NOT fail any request or liveness probe, and SHALL NOT alter any grant decision — the Channel gate of criterion 19.4 is unchanged; what is reported is a configuration that has become unsatisfiable.
...
6a. **(Clarified, task 32 — A18.)** Criteria 2, 3, 4 and 6 are **structural**: they require the configuration to EXIST and to be extensible, not that the engine read it at runtime. At MVP the engine reads the hardcoded GBP constants (`DEFAULT_TIER_RULES`, `REWARDS`), and the `markets` / `earning_rule_sets` / `reward_rule_sets` tables plus `DbMarketConfigProvider` are the retained, tested forward path for a second Market. Because that leaves two representations of the same rules with only one obeyed, THE Loyalty_Service SHALL publish, read-only, whether the configured rule-set rows still agree with the constants the engine uses, so a diverged table is observable rather than misleading. A divergence SHALL NOT fail any request or liveness probe, since no engine decision depends on those rows. Changing a threshold, multiplier or reward cost at MVP therefore requires a deploy.
