...
- [ ] 26. Rehearse the M0–M2 migration/cutover runbook
  - **Rehearsed on staging (2026-07-26); NOT signed off — blocked on task 33.** The runbook exists with per-step evidence at `docs/ops/m0-m2-cutover-runbook.md`. M0 passed on staging including both failure paths (incomplete export aborts before any change with no backup written; a balance-formula violation halts but still writes the anchor first). M1 passed against an isolated scratch Postgres — one `migration` entry and one non-expiring lot per enrolled customer, `SUM(ledger) == exported balance` for each, tier recomputed, non-enrolled deferred (0 rows), idempotent on re-run, and both a mid-way failure and a forced reconciliation mismatch rolled back to zero retained state. Rollback passed: refuses to run while the service is up without touching a single metafield, then restores every customer's metafields exactly to their exported values, idempotently. No metafield was deleted at any point. Migration over HTTP confirmed still refused with 501 `migration_not_enabled`. Staging was restored to its prior state.
  - **Why it is not signed off:** the rehearsal had to supply the production boundaries itself. No concrete `MigrationShopifyClient`, no `MetafieldRestoreClient`, no `ServiceController`, and no operator entrypoint script exist (tracked as task 33, and recorded as occurrence 5 in `docs/ops/reachability-audit.md`). "Confirm the operator-script path works" is therefore not satisfiable yet. Two phases also remain unproven: **M2 was not rehearsable** — the staging token lacks `read_themes` and the staging storefront is a bare dev store behind a password page, so no dashboard cutover evidence exists — and **lifetime-spend derivation from paid orders is entirely untested**, because staging has zero paid orders and the rehearsal supplied spend from a fixture. That derivation does not exist as code yet and is the piece most likely to be wrong in production.
...
- [ ] 33. Build the migration operator tooling (blocks task 26 sign-off and the real cutover)
  - **Reachability audit occurrence 5.** The M0–M2 migration logic is implemented and now proven by the task 26 rehearsal, including every abort and halt path — but nothing connects it to a real store or a real operator, so the cutover cannot be executed. Tasks 7.1–7.3 were marked complete on the pure logic plus unit tests that inject their own fakes. Build the missing boundaries:
    - A concrete `MigrationShopifyClient` (Admin GraphQL): list every customer with their `loyalty.*` metafields, and **derive lifetime spend from paid orders**. The spend derivation is the highest-risk untested piece in the whole cutover — the rehearsal could only supply it from a fixture, because staging has no paid orders. It needs its own tests against real staging order data.
    - A concrete `MetafieldRestoreClient` for rollback (`metafieldsSet` upsert plus read-back verification). It must have no delete path, so Req 14.8 is structurally guaranteed rather than merely observed.
    - Operator entrypoint scripts under `loyalty-service/scripts/migration/` for M0, M1 and the metafield rollback, with npm scripts, argument validation, and an explicit guard against pointing at the wrong store domain. Keep migration refused over HTTP (Req 10.7a) — these are deliberately operator-only.
    - A production `ServiceController`, or an explicit decision that suspending the service stays a documented manual Render step (the rollback code already fails closed if the service is still up, so a manual step is defensible — but it should be a decision, not an omission).
  - Then re-run the task 26 rehearsal through the committed scripts rather than a harness, and sign the runbook off.
...
- Task 33 (migration operator tooling) came out of the task 26 rehearsal and **blocks task 26 sign-off and the real cutover**. It is the only Phase 5 dependency besides task 28's on task 24.
