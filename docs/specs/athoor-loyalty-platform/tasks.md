...
- [ ] 36. Assign referral codes to members who never got one
  - **Found while implementing task 34, outside its scope.** Only 2 of the 8 enrolled staging customers have a `customers.referral_code`; the six created before the task-25 wiring landed have none. `assignReferralCode` runs solely on the `customers/create` signup path and nothing assigns a code lazily afterwards, so for those members `GET /v1/referral` returns `referralCode: null`, the metafield writer correctly writes nothing, and the dashboard shows the "Preparing your code…" placeholder **indefinitely** — they can never refer anyone.
  - Fix: assign lazily on first read in `GET /v1/referral` (idempotent, so concurrent reads cannot mint two codes) and/or add a one-off backfill for existing members, following the pattern of `scripts/backfill-missing-point-lots.mjs` (dry-run by default, idempotent, atomic). Decide which; lazy assignment alone leaves the metafield cache empty until the member visits, so a backfill plus a cache refresh may be wanted. Verify on staging that all enrolled members end up with a code and that repeat reads do not change it.
  - _Requirements: 2.9, 11.8_
...
- Task 33 (migration operator tooling) came out of the task 26 rehearsal and unblocked its sign-off. Remaining open Phase 5 work: 27 (dashboard/Lighthouse/accessibility, blocked on storefront prerequisites, and also the source of the outstanding M2 evidence for task 26), 30, 31, 32, 34, 35.
