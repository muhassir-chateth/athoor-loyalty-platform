...
- [ ] 39. Prevent circular and collusive referrals
  - **Found during the task 38 live verification, outside its scope.** Self-referral is guarded (Req 11.8, Property 12, enforced by the `referred_by` check and a DB `CHECK`), but a **two-party loop is not**: A submitted B's code and B submitted A's code, and **both returned `rewarded`**. Each customer's `referred_by` now points at the other and each referrer was credited +150 with a backing lot. Two colluding accounts — or one person with two accounts — mint 300 points for nothing, and the +250 first-purchase stage would follow.
  - This is referral fraud rather than a ledger-integrity defect: every entry is correctly recorded and backed. The rules simply permit a cycle.
  - Decide the policy before fixing, since it is a business rule and not purely technical: refuse a claim when the referrer was themselves referred by the claimant (the minimal cycle), or refuse any cycle of any length, or allow the claim but withhold the reward, or cap rewards per household/payment instrument. Note the existing fraud-review surface (`GET /v1/admin/fraud-review`, Req 10.6) already lists referrals and would surface these once detectable, so detection and refusal are separable.
  - Whatever is chosen, add a property test alongside Property 12 asserting the invariant, and check whether existing staging/production data contains cycles before enforcing.
  - _Requirements: 2.9, 2.9a, 11.8, 10.6_ `storageKeyFor` in `src/plugins/idempotency.ts` namespaces the stored response by `METHOD route:clientKey` only — the authenticated customer is **not** in the key. The client fully controls that header value, so if two customers use the same key on the same route within the 24-hour window, the second receives the **first customer's stored response verbatim and their own operation never executes**. A guessable value like `1` or `test` is enough; this is low-complexity and does not require a collision.
